    ...
    if ($action eq "update") {
        ...
        if ($tree eq 'name' || $tree eq 'ipv6-name') {
            ...
            foreach my $zone (@all_zones) {
                my @zone_interfaces =Vyatta::Zone::get_zone_interfaces("returnValues", $zone);
        ...
        if (!defined($tree2)) {
            ...
            print STDERR 'Configuration error: ' ."Rule set \"$chain\" is not configured\n";
            exit 1;
        }
...
sub update_rules {
    ...
    if ($nodes{$name} eq 'static') {
        ...
        if (($policy_log) and (length($name) > 19)) {
            ...
            printf STDERR 'Configuration warning: '. "default logging prefix will be truncated to \"[$chain_tr-DEFLT-$action_char]\" \n";
        }
        ...
        if (!($test)) {
            if (($policy_log) and (length($name) > 19)) {
                my $action_char = uc(substr($policy, 0, 1));
                my $chain_tr = substr($name, 0, 19);
                printf STDERR 'Configuration warning2: '. "default logging prefix will be truncated to \"[$chain_tr-DEFLT-$action_char]\" \n";
            }
        }
    ...
    if (  (defined $old_policy and $policy ne $old_policy)
        or($old_policy_log ne $policy_log))
    {
        change_default_policy($table, $name, $iptables_cmd, $policy,$old_policy_log,$policy_log);
    }
...
sub update_ints {
    ...
    if (   !defined $action
        ...
        || !defined $chain
        || !defined $table
        || !defined $iptables_cmd)
    {
        return -1;
    }

    if ($action ne 'delete' && $table eq 'mangle' && $direction =~ /^local/) {
        print STDERR 'Configuration error: ' ."\"Modify\" rule set \"$chain\" cannot be used for " ."\"local\"\n";
...
CASE: {
    ...
    my @lines= `$iptables_cmd -t $table -L $direction -n -v --line-numbers | $grep`;
    my ($cmd, $num, $oldchain, $in, $out, $ignore)= (undef, undef, undef, undef, undef, undef);
    ...
    foreach (@lines) {
        ...
        if ($iptables_cmd eq "iptables") {
            ($num, $ignore, $ignore, $oldchain, $ignore, $ignore, $in, $out,$ignore, $ignore) = split /\s+/;
        } else {
            ($num, $ignore, $ignore, $oldchain, $ignore,  $in, $out,$ignore, $ignore) = split /\s+/;
        }
...
sub setup_chain {
    ...
    if (!/^Chain $chain/) {
        ...
        printf STDERR 'Configuration error: '. "Chain \"$chain\" being used in system. Cannot use it as a ruleset name\n";
        exit 1;
    }
}
